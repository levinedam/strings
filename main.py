...
async def send_success_message(message: Message, session_string: str, account_info: str, current_state: dict):
    ...
    # Membuat teks String Session lengkap untuk tombol inline
    session_details_text = (
        f"**{session_type.upper()} Userbot Client**\n"
    ...
    # Teks yang ditampilkan kepada pengguna (hanya berisi instruksi dan tombol)
    final_text = (
        ...
        "Tekan tombol **String Session** di bawah untuk melihat dan menyimpan detail sesi Anda di Pesan Tersimpan (Saved Messages)."
    )
    
    # Tombol yang berisi URL untuk mengirimkan semua detail sesi ke Pesan Tersimpan
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("➡️ String Session", url=f"tg://msg?text={session_details_text}&to=me")]
    ])
